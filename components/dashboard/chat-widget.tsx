...
export function ChatWidget({ activeChar, updateCharacter }: ChatWidgetProps) {
    ...
    const clearChat = () => {
        if (confirm(t('dashboard.chat.clear'))) {
    ...
    return (
        <BentoCard className="h-full flex flex-col">
            <div className="flex justify-between items-center mb-2">
                <BentoTitle icon={MessageSquare}>{t('dashboard.chat.title')}</BentoTitle>
                <button onClick={clearChat} className="p-1 hover:text-red-500 text-muted-foreground transition-colors" title={t('common.delete')}>
            ...
            <div className="flex-1 overflow-y-auto mb-2 space-y-2 pr-1 text-sm custom-scrollbar" ref={scrollRef}>
                {messages.length === 0 && (
                    <div className="text-center text-muted-foreground italic mt-4 text-xs">
                        {t('dashboard.chat.start')} {activeChar.name}...
                    </div>
                )}
                ...
                {isLoading && (
                    <div className="flex justify-start">
                        <div className="bg-zinc-800 rounded-lg px-3 py-1.5 flex items-center">
                            ...
                            <span className="text-xs">{t('dashboard.chat.thinking')}</span>
                        </div>
                    </div>
                )}
            </div>
